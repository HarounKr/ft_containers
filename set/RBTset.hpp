...
#include <iostream>
#include <limits>
#include "../iterator/setIterator.hpp"
...
template < class key, class mapped_type, class Compare = std::less<key>, class Alloc = std::allocator<Node< key, mapped_type> > >
	class	RBtree {
		...
		public :
			...
			typedef	ft::setIterator<const Node, const key>									iterator;
			typedef	ft::setIterator<const Node, const key>						const_iterator;
		...
		public:
			...
			template <class InputIterator>
  			void insert (InputIterator first, InputIterator last) {
				while (first != last){
					insert(ft::make_pair(*first, *first));
					first++;
				}
			}
			...
			void erase(iterator position) {
				if (this->_size > 0)
					this->delete_Node(ft::make_pair(*position, *position));
			}
			...
			iterator search_val(const value_type & value) const {
				Node* tmp = this->_rbt.root;
